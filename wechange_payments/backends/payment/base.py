...
class BaseBackend(object):
    ...
    def user_pre_recurring_payment_safety_checks(self, user):
        ...
        almost_a_year = now() - timedelta(days=360)
        paid_payments_month = Payment.objects.filter(user=user, status=Payment.STATUS_PAID, completed_at__gt=almost_a_year)
        ...
        if payment_sum > settings.PAYMENTS_HARDCAP_RECURRING_YEARLY_ACCOUNT_AGGREGATED_PAYMENT_SUM:
            logger.critical('Payments: NEED TO INVESTIGATE! HARDCAP_RECURRING_YEARLY_ACCOUNT_AGGREGATED_PAYMENT_SUM was exceeded in `user_pre_recurring_payment_safety_checks` was prevented because the sum of Payment amounts for this user in the last 28 days exceeds the maximum hardcap payment amount!',
