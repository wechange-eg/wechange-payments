...
class TrytonInvoiceBackend(LexofficeInvoiceBackend):
    ...
    def _make_invoice_request_params(self, invoice):
        ...
        if settings.PAYMENTS_INVOICE_PORTAL_PARTEI_IDENTIFIKATOR_KEY and settings.PAYMENTS_INVOICE_PORTAL_ID:
            ...
            data['address'].update({
                'identifiers': [{'type': settings.PAYMENTS_INVOICE_PORTAL_PARTEI_IDENTIFIKATOR_KEY, 'code': self.get_customer_portal_id(invoice), }]
            })
        return data
